...
export function TodayRadar({ items }: TodayRadarProps) {
  return (
    <section id="today-radar" className="today-radar premium-card">
      ...
      <ul className="today-radar-list">
        {items.map((project, index) => (
          <li key={project.slug}>
            ...
            <strong aria-label={`${project.name} 站内评分 ${getProjectOverallScore(project)}`}>
              <Flame size={13} aria-hidden="true" /> 站内评分 {getProjectOverallScore(project)}
            </strong>
          </li>
        ))}
      </ul>
